...
VS_OUTPUT main(uint vertex_id : SV_VertexID, uint instance_id : SV_InstanceID)
{
	...
	BillboardWindResult wind = ApplyBillboardWind(
		world_pos, height_factor,
		g_Perlin4D, smp_linear,
		g_wind_direction, wave,
		grass_wind_displacement);
	world_pos = wind.world_pos;

	O.position = mul(g_LightVP, world_pos);
	O.texcoord = float2(v.u, v.v);
	return O;
}
